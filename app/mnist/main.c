...
int main(int argc, char *argv[]) {
    ...
    switch (cmd_args.mode) {
        case MODE_TRAIN: {
            mnist_train(cmd_args.model_filename, cmd_args.iterations, cmd_args.do_overwrite);
            return 0;
        }
        case MODE_TEST: {
...
void read_args(cmd_args_t *cmd_args, int argc, char *argv[], int *argi) {
    ...
    *argi += 1;
    ...
    if (arg_matches(arg, "--mode", "-h")) {
        ...
        *argi += 1;
        if (arg_matches(arg, "--help", "-h")) {
            printf("Available modes: 'train', 'test'.\nExample usage: --mode train --load-file\n");
            exit(EXIT_SUCCESS);
            return;
        }
    ...
    if (arg_matches(arg, "--iterations", "-i")) {
        cnd_make_error(*argi == argc, "Expected another argument. Use '--iterations --help' to find out more.\n");
        arg = argv[*argi];
        *argi += 1;
        if (arg_matches(arg, "--help", "-h")) {
            printf("The number of times each test case is iterated over during training.\nExample use: --mode train --iterations 5\n");
        }
        cmd_args->iterations = atoi(arg);
        cnd_make_error(cmd_args->iterations <= 0, "Inputted string for iterations is not a valid number.\n");
        return;
    }
